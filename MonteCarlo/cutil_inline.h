...
inline void __cudaSafeCall(cudaError_t err,
...
inline void cutilDeviceInit(int argc, char **argv) {
  ...
  if (argc == 2)
  {
	  printf("\n Setting CUDA device %s\n", argv[1]);//  if you have multiple devices you can select one
...
//
// windows timing routine
//

#include <time.h>

inline double elapsed_time(clock_t *et) {
  clock_t old_time = *et;  
 *et = clock();

 return (double)(*et - old_time) / CLOCKS_PER_SEC *1.0e3;
}
